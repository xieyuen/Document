...
class Tools:
    ...
    class Math:
        ...
        def sum(_expressions,_start=0,_stop=10):
            ...
            class baomi: # www.baomi.org.cn
                ...
                class mainThread (threading.Thread):   #继承父类threading.Thread
                    ...
                    def run(self):
                        ...
                        # 创建子线程
                        thread1 = Script.AutoStudy.baomi.childThread(1, "chrome", self.url)
                        thread2 = Script.AutoStudy.baomi.childThread(2, "play", self.url)
                ...
                def main_program(x = 230, y = 760, typing = 'edge', kill = None):

                    """
                        你至少需要提供两个参数作为鼠标点击位置(即播放按钮的坐标)
                        这两个参数的默认值为 230, 760
                        在这之后还有一个参数作为浏览器的指示，暂时仅支持 Edge 和 Chrome 的自动开启和关闭
                        需要其他的浏览器可以选择在参数末尾加上 `kill='浏览器进程名'`
                        浏览器进程名一般后面有 `.exe`
                        For example:
                        1)
                    ...
                    for i in Script.AutoStudy.baomi.__urlAndTime:
                        ...
                        # 创建子线程
                        thread1 = Script.AutoStudy.baomi.childThread(1, "chrome", url)
                        thread2 = Script.AutoStudy.baomi.childThread(2, "play", url)
